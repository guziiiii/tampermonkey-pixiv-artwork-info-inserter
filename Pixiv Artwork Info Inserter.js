...
(function () {
  ...
  GM_addStyle(`
    ...
    #pixiv-info-inserter .pii-group {
      background: linear-gradient(145deg, rgba(53,120,192,0.12), rgba(42,95,160,0.08));
      border-radius: 10px;
      padding: 12px 16px;
      margin-bottom: 14px;
    }
    [data-theme="dark"] #pixiv-info-inserter .pii-group {
      background: linear-gradient(145deg, rgba(42,100,170,0.15), rgba(30,78,145,0.1));
    }

    /* === 按钮组 === */
    #pixiv-info-inserter .pii-btn-group {
      background: linear-gradient(145deg, rgba(53,120,192,0.10), rgba(45,105,172,0.06));
      border-radius: 10px;
      padding: 12px 16px;
      display: flex;
      flex-wrap: wrap;
    ...
    [data-theme="dark"] #pixiv-info-inserter .pii-btn {
      background: rgba(70,140,210,0.7);
      border-color: rgba(255,255,255,0.12);
    }
    [data-theme="dark"] #pixiv-info-inserter .pii-btn:hover {
      background: rgba(70,140,210,0.45);
    }
  ...
  function makeLine(label, value) {
    ...
    line.addEventListener("click", function (e) {
      e.stopPropagation();
      var copiedText = Array.isArray(value) ? value.join("\n") : String(value);
      copyText(copiedText);
      line.classList.add("flash");
      showToast(line, "\u2714 \u5DF2\u590D\u5236");
      setTimeout(function () { line.classList.remove("flash"); }, 1200);
    });
  ...
  function createCopyAllLine(infoGroup) {
    var line = document.createElement("div");
    line.className = "pii-copy-all";
    line.title = "\u70B9\u51FB\u590D\u5236\u6240\u6709\u4FE1\u606F";

    var textSpan = document.createElement("span");
    textSpan.textContent = "\u4E00\u952E\u590D\u5236\u6240\u6709\u4FE1\u606F";
    ...
    line.addEventListener("click", function (e) {
      e.stopPropagation();
      var resultLines = [];
      infoGroup.querySelectorAll(".pii-line").forEach(function (l) {
        var span = l.querySelector("span");
        if (span && span.textContent.indexOf(":") !== -1) {
          resultLines.push(span.textContent);
        }
      });
      copyText(resultLines.join("\n"));
      iconSpan.textContent = "\u2705";
      showToast(line, "\u2714 \u6240\u6709\u4FE1\u606F\u5DF2\u590D\u5236");
      setTimeout(function () { iconSpan.textContent = "\uD83D\uDCCB"; }, 1200);
    });
  ...
  function extractData() {
    ...
    if (h1 && h1.textContent && h1.textContent.trim()) {
      ...
      title = t[0] ? t[0].replace(/^#[^\s]+\s*/, "").trim() : title;
    }

    var authorName = "\u672A\u77E5\u4F5C\u8005";
    var tParts = document.title.split(" - ");
    if (tParts.length > 1) authorName = tParts[1].trim();
    // 清理 Pixiv 国际化页面在作者名后追加的品类后缀
    // 中文：的插画/的动图/的漫画/的小说/的投稿/的模写/的作品/的文章/的創作
    // 日文：のイラスト/のマンガ/のうごイラ/の作品/の絵
    ...
    for (var i = 0; i < anchors.length; i++) {
      var m = anchors[i].getAttribute("href").match(/\/users\/(\d+)/);
      if (m) { authorUid = m[1]; break; }
    }
  ...
  // ===================================================================
  // 5. 🧩 面板 Builder：收起按钮 / 展开面板
  // ===================================================================
  function buildWidget(data) {
    var widget = document.createElement("div");
    widget.id = "pixiv-info-inserter";

    // ── 收起状态按钮 ──
    var fab = document.createElement("div");
    fab.className = "pii-btn-fab";
    fab.innerHTML = "\uD83D\uDCCB PIXIV";

    // ── 展开状态面板 ──
    var panel = document.createElement("div");
    panel.className = "pii-panel";
    ...
    headerTitle.className = "pii-header-title";
    headerTitle.innerHTML = "\uD83D\uDCCB Pixiv Info";

    var closeBtn = document.createElement("button");
    closeBtn.className = "pii-header-close";
    closeBtn.innerHTML = "\u2715";
    closeBtn.title = "\u6536\u8D77\u9762\u677F";
    ...
    body.className = "pii-body";

    var infoGroup = document.createElement("div");
    infoGroup.className = "pii-group";
    infoGroup.appendChild(makeLine("\u4F5C\u54C1\u540D\u79F0", data.title));
    infoGroup.appendChild(makeLine("PID", data.pid));
    infoGroup.appendChild(makeLine("\u4F5C\u8005\u540D", data.authorName));
    infoGroup.appendChild(makeLine("UID", data.authorUid));
    infoGroup.appendChild(createCopyAllLine(infoGroup));

    var buttonGroup = document.createElement("div");
    buttonGroup.className = "pii-btn-group";
    buttonGroup.appendChild(
      makeButton("uid ->", function () {
        window.open("https://exhentai.org/?f_search=" + encodeURIComponent(data.authorUid), "_blank");
      })
    );
    buttonGroup.appendChild(
      makeButton("name->", function () {
        window.open("https://exhentai.org/?f_search=" + encodeURIComponent(data.authorName), "_blank");
      })
    );
  ...
  function setupDrag(widget, fab, header) {
    // 恢复上次位置
    var savedLeft = parseFloat(GM_getValue("piiLeft", "NaN"));
    var savedTop = parseFloat(GM_getValue("piiTop", "NaN"));
    ...
    function onStart(e) {
      var target = e.target;
      if (target.closest && (target.closest(".pii-line") || target.closest(".pii-btn") || target.closest(".pii-header-close") || target.closest(".pii-copy-all"))) {
        return;
      }
      ...
      widget.classList.add("pii-dragging");
      e.preventDefault();
    }
    ...
    function onMove(e) {
      ...
      if (dragState.moved) {
        ...
        var maxLeft = window.innerWidth - widget.offsetWidth - 10;
        var maxTop = window.innerHeight - widget.offsetHeight - 10;
    ...
    function onEnd() {
      if (dragState) {
        widget.classList.remove("pii-dragging");

        if (dragState.moved) {
          var rect = widget.getBoundingClientRect();
          GM_setValue("piiLeft", rect.left);
          GM_setValue("piiTop", rect.top);
        }
  ...
  // ===================================================================
  function inject() {
    if (document.getElementById("pixiv-info-inserter")) return true;
    if (!document.body) return false;

    var data = extractData();
    var widget = buildWidget(data);
    var fab = widget.querySelector(".pii-btn-fab");
    var header = widget.querySelector(".pii-header");

    document.body.appendChild(widget);
    setupDrag(widget, fab, header);

    console.log("[Pixiv Info] Widget appended to body");
    return true;
  }
  ...
  function start() {
    console.log("[Pixiv Info] Starting...");
    ...
    setTimeout(function () {
      if (!_injected) {
        console.warn("[Pixiv Info] Giving up after 30s.");
  ...
  var spaObserver = new MutationObserver(function () {
    if (location.pathname !== lastPathname) {
      console.log("[Pixiv Info] SPA route changed:", lastPathname, "->", location.pathname);
      lastPathname = location.pathname;

      var old = document.getElementById("pixiv-info-inserter");
      if (old) old.remove();
